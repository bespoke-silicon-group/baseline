...
__attribute__ ((noinline)) int edgeset_apply_pull_parallel_from_vertexset_to_filter_func_with_frontier(int *in_indices , int *in_neighbors, int * frontier, int * next_frontier, int * parent, int V, int E, int part, int block_size_x) 
{

  bsg_cuda_print_stat_start(part);
  int start, end;
  int start_vertex = (V/PARTITIONS)*part;
  int end_int = start_vertex + (V/PARTITIONS);
  int end_vertex = (end_int) > V ? V : end_int;
  local_range_partial(start_vertex, end_vertex, &start, &end);
  if(bsg_id == 0) pr_dbg("vertex range: %i - %i , my local range: %i - %i\n", start_vertex, end_vertex, start, end);
...
//            traversed++;
            ...
            //int src = neighbors[s];
            if(frontier[neighbors[s]] == 1) {
            //if( apply_func( src, d )) { 
              parent[d] = neighbors[s];
              next_frontier[d] = 1;
	      break;
              //if(bsg_id == 0) { iter++; } 
            }
...
extern "C" int __attribute__ ((noinline)) edgeset_apply_pull_parallel_from_vertexset_to_filter_func_with_frontier_call(int *in_indices, int *in_neighbors, int * frontier, int * next_frontier, int * parent, int V, int E, int part, int block_size_x) {
        barrier.sync();
        //bsg_cuda_print_stat_start(1);
        //pr_dbg("before call to kernel\n");
        edgeset_apply_pull_parallel_from_vertexset_to_filter_func_with_frontier(in_indices, in_neighbors, frontier, next_frontier, parent, V, E, part, block_size_x);
