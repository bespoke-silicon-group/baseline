...
__attribute__ ((noinline)) int edgeset_apply_pull_parallel_from_vertexset_to_filter_func_with_frontier(int *in_indices , int *in_neighbors, int * frontier, int * next_frontier, int * parent, int V, int E, int block_size_x) 
{

  bsg_cuda_print_stat_start(1);
  int start, end;
  local_range((V/PARTITIONS), &start, &end);
  //bsg_cuda_print_stat_start(1);
  int traversed = 0;
...
extern "C" int __attribute__ ((noinline)) edgeset_apply_pull_parallel_from_vertexset_to_filter_func_with_frontier_call(int *in_indices, int *in_neighbors, int * frontier, int * next_frontier, int * parent, int V, int E, int block_size_x) {
        ...
        edgeset_apply_pull_parallel_from_vertexset_to_filter_func_with_frontier(in_indices, in_neighbors, frontier, next_frontier, parent, V, E, block_size_x);
        //bsg_cuda_print_stat_end(1);
    barrier.sync();
	  return 0;
}
