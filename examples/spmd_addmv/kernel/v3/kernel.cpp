...
inline void load_block(float * bsg_attr_noalias dest,
                ...
                uint32_t * bsg_attr_noalias src_strides,
                int by_i, int bx_i) {
...
inline void load_col(float * bsg_attr_noalias dest,
        ...
        // Move the raw pointer to the row/column start.
        //Generic: src = src + (b_i * B * src_strides[0]);
        src = src + (b_i * B);
...
inline void store_block_and_reset(float * bsg_attr_noalias src,
                           ...
                           uint32_t * bsg_attr_noalias dest_strides,
                           int by_i) {
...
inline void accum_col(float* bsg_attr_noalias dest,
        ...
        for (int bx_i = 0; bx_i < BX; ++bx_i) {
                ...
                for(int by_i = 0; by_i < BY; ++by_i) {
                        ...
                        psum[by_i] = fmaf(col,row_anchor[bx_i], psum[by_i]);
                }
        }
...
int kernel_addmv(float bsg_attr_remote * bsg_attr_noalias result,
        ...
        for (int by_i = by_start; __builtin_expect(by_i < by_end, 1); by_i++){
                float bsg_attr_remote * bsg_attr_noalias input_p = &input[by_i * BY];

                for (int bx_i = 0; __builtin_expect(bx_i < blocks, 1); bx_i++){
                ...
                bsg_unroll(16)
                for (int sby_i = 0; sby_i < BY; ++sby_i) {
                        // Can't get input_p to be read earlier... or be register allocated
                        psum[sby_i] = fmaf(input_p[sby_i], beta, psum[sby_i] * alpha);
                }
...
int kernel_addmv(
        ...
        kernel_addmv<4, 4, false>(result_p,
                                  result.get_strides(),
                                  input_p,
                                  input.get_strides(),
                                  mat_p,
