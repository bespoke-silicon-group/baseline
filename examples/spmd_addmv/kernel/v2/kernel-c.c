...
int kernel_addmv_c(float bsg_attr_remote * bsg_attr_noalias result,
        ...
        for (int by_i = by_start; by_i < by_end; by_i++){
                load_col(block_input, input, input_strides, by_i);
                bsg_unroll(1)
                for (int bx_i = 0; bx_i < blocks; bx_i++){
                        load_block(block_rows, mat, mat_strides, by_i, bx_i);
                        load_col(block_col, vec, vec_strides, bx_i);
                        accum_col(psum, block_rows, block_col);
                }
                ...
                store_block_and_reset(psum, result, result_strides, by_i);
        }
