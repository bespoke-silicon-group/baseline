...
int launch(int argc, char * argv[]){
  ...
  double tmp_rank_val = (double) 1 / edges.num_edges();
  std::vector<double> zerosf(edges.num_nodes(), 0.0);
  std::vector<int32_t> zeros(edges.num_nodes(), 0);
  std::vector<double> rank(edges.num_nodes(), tmp_rank_val);
  
  std::cerr << "load and init vectors " << tmp_rank_val << "\n";
  old_rank_dev = Vector<double>(edges.num_nodes());
  new_rank_dev = Vector<double>(edges.num_nodes());
  out_degree_dev = Vector<int32_t>(edges.num_nodes());
  contrib_dev = Vector<double>(edges.num_nodes());
  error_dev = Vector<double>(edges.num_nodes());
  ...
  for ( int i = (0) ; i < (1) ; i++ )
  {
    ...
    device->runJobs();
    device->enqueueJob("edgeset_apply_pull_serial_call", hb_mc_dimension(16,8),{edges.getOutIndicesAddr() , edges.getOutNeighborsAddr(), new_rank_dev.getAddr(), contrib_dev.getAddr(), edges.num_nodes(), edges.num_edges()});
    device->runJobs();
