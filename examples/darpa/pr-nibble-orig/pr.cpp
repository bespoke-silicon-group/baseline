...
int launch(int argc, char * argv[]){
  ...
  std::vector<double> zerosf(edges.num_nodes(), 0.0);
  ...
  hammerblade::write_global_buffer_dma<double>(zerosf.data(), p_dev, zerosf.size());  
  hammerblade::write_global_buffer_dma<double>(zerosf.data(), old_rank_dev, zerosf.size());  
  hammerblade::write_global_buffer_dma<double>(zerosf.data(), new_rank_dev, zerosf.size());  
  ...
  //while ( iter < 16) 
  {
	...
	std::cerr << "pull kernel\n";
        ...
        device->enqueueJob("edgeset_apply_pull_parallel_from_vertexset_call", hb_mc_dimension(X,Y),{edges.getInIndicesAddr() , edges.getInNeighborsAddr(), frontier.getAddr(), edges.num_nodes(), edges.num_edges(), edges.num_nodes(), iter});
    	device->runJobs();
	...
	std::cerr << "push kernel\n";
    	...
    	tag_c++;
    	std::cerr << "run update edges kernel on iter : " << iter << "\n";
        device->enqueueJob("edgeset_apply_push_parallel_from_vertexset_call", hb_mc_dimension(X,Y),{edges.getOutIndicesAddr() , edges.getOutNeighborsAddr(), frontier.getAddr(), edges.num_nodes(), edges.num_edges(), edges.num_nodes(), iter}); 
    	device->runJobs();
    	std::cerr << "create next frontier\n";
    	device->enqueueJob("filter_frontier_where_call", hb_mc_dimension(X,Y),{frontier.getAddr(), edges.num_nodes(), edges.num_edges(), tag_c});
    	device->runJobs();
    ...
    tag_c++;
    std::cerr << "copy new rank to old rank: \n";
    //old_rank = new_rank;
