...
extern "C" int __attribute__ ((noinline)) edgeset_apply_push_serial_from_vertexset_to_filter_func_with_frontier_call(int *out_indices, int *out_neighbors, int *frontier, int *next_frontier, int V, int E, int block_size_x) {
  barrier.sync();
  bsg_cuda_print_stat_start(1);
	edgeset_apply_push_serial_from_vertexset_to_filter_func_with_frontier(out_indices, out_neighbors, frontier, next_frontier, toFilter(), updateEdge(), V, E, block_size_x);
