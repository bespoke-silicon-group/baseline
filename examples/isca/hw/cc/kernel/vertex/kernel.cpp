...
template <typename APPLY_FUNC > int edgeset_apply_push_parallel_deduplicatied_with_frontier(int *out_indices , int *out_neighbors, int * next_frontier, APPLY_FUNC apply_func, int V, int E, int block_size_x) 
{ 
  int start, end;
...
extern "C" int __attribute__ ((noinline)) edgeset_apply_push_parallel_deduplicated_with_frontier_call(int *out_indices, int *out_neighbors, int *next_frontier, int V, int E, int block_size_x) {
        barrier.sync();
        bsg_cuda_print_stat_start(0);
  	edgeset_apply_push_parallel_deduplicatied_with_frontier(out_indices, out_neighbors, next_frontier, updateEdgeAtomic(), V, E, block_size_x);
