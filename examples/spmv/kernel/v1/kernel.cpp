...
/* 
 * Version 0 
 *  Matrix X/Y dimensions as well as vetor dimensions are templatized
 *  to allow for better compiler optimization.
 */
template <int MATRIX_HEIGHT, int MATRIX_WIDTH, int VECTOR_LENGTH, typename TM, typename TV, typename TR>
int  __attribute__ ((noinline)) spmv_template_dimensions (TM* sparse_matrix, 
...
extern "C" {
        int  __attribute__ ((noinline)) kernel_spmv (int* sparse_matrix, 
                ...
                rc = spmv_template_dimensions<64,64,32> (sparse_matrix,
                                                        dense_vector_index,
                                                        dense_vector_data,
