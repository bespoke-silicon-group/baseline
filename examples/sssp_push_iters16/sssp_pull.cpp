...
int launch(int argc, char * argv[]){
	...
	for(int i = 0; i < 1; i++)
  {
    device->enqueueJob("edgeset_apply_push_serial_weighted_deduplicatied_from_vertexset_with_frontier_call", 
												...
												{edges.getOutIndicesAddr() , edges.getOutNeighborsAddr(), frontier.getAddr(), next_frontier.getAddr(), SP_dev.getAddr(), edges.num_nodes(), edges.num_edges(),edges.num_nodes()});
    device->runJobs();
