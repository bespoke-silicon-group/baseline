...
int launch(int argc, char * argv[]){
  ...
  int * frontier_h = hammerblade::builtin_loadFrontierFromFile(frontier_f.c_str());

  std::string state_f = input.getCmdOption("-s");
  int * sp_h = hammerblade::builtin_loadFrontierFromFile(state_f.c_str());
  ...
  {
    if(push) {
      device->enqueueJob("edgeset_apply_push_serial_weighted_deduplicatied_from_vertexset_with_frontier_call",
		hb_mc_dimension(16,8),									  {edges.getOutVertexlistAddr() , edges.getOutNeighborsAddr(), frontier.getAddr(), next_frontier.getAddr(), SP_dev.getAddr(), edges.num_nodes(), edges.num_edges(),edges.num_nodes()});

    } else {
      device->enqueueJob("edgeset_apply_pull_serial_weighted_deduplicatied_from_vertexset_with_frontier_call",
		hb_mc_dimension(16,8),									  {edges.getInVertexlistAddr() , edges.getInNeighborsAddr(), frontier.getAddr(), next_frontier.getAddr(), SP_dev.getAddr(), edges.num_nodes(), edges.num_edges(),edges.num_nodes()});
    }
    device->runJobs();
