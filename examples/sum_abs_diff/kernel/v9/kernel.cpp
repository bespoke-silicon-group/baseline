...
/*
 * Version 9 - Using new templatized barrier library 
 * Similar to version 8, but uses templatized barrier library instead 
 */
template <int FRAME_HEIGHT, int FRAME_WIDTH>
int  __attribute__ ((noinline)) sum_abs_diff_frame_sh_mem_striped_barrier_template  (int *REF, int *FRAME, int *RES,
...
extern "C" {
        int  __attribute__ ((noinline)) kernel_sum_abs_diff(
                ...
                rc = sum_abs_diff_frame_sh_mem_striped_barrier_template <4,4>   (REF, FRAME, RES,
