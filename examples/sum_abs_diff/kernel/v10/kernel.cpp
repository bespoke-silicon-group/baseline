...
/*
 * Version 10 - Most optimized version with tile group shared memory
 * Templatized both frame and reference dimensions 
 * 
 */
template <int REF_HEIGHT, int REF_WIDTH, int FRAME_HEIGHT, int FRAME_WIDTH>
int  __attribute__ ((noinline)) sum_abs_diff_ref_frame_template_frame_sh_mem_striped_barrier_template  (int *REF, int *FRAME, int *RES,
        ...
        bsg_barrier <bsg_tiles_Y, bsg_tiles_X> my_barrier;
...
extern "C" {
        int  __attribute__ ((noinline)) kernel_sum_abs_diff(
                ...
                rc = sum_abs_diff_ref_frame_template_frame_sh_mem_striped_barrier_template <16,16,4,4>   (REF, FRAME, RES,
